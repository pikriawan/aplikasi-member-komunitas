        ...
        <aside :class="cn('fixed top-0 -left-full lg:static lg:top-auto lg:left-auto lg:row-span-2 z-2 w-full lg:w-auto h-full lg:h-auto flex flex-col bg-white shadow-[-0.0625rem_0_0_var(--color-onyx-200)_inset] transition-[left_var(--default-transition-timing-function)_var(--default-transition-duration)]', sidebarVisible && 'left-0')">
            ...
            <div class="w-full h-full flex flex-col justify-between p-8">
                <nav class="w-full flex flex-col">
                    <Link :class="cn('w-full flex items-center gap-4 p-4 rounded-lg font-medium', route().current() === 'staff.contents.index' && 'bg-surface text-primary')" :href="route('staff.contents.index')">
                    ...
                    <Link :class="cn('w-full flex items-center gap-4 p-4 rounded-lg font-medium', route().current() === 'staff.conversations.index' && 'bg-surface text-primary')" :href="route('staff.conversations.index')">
                ...
                <div class="w-full flex flex-col gap-4">
                    <Popover>
                        ...
                        <PopoverContent class="bottom-full left-0 flex flex-col gap-px bg-onyx-200">
                            <Link class="flex items-center gap-4 p-4 bg-white first:rounded-t-[0.4375rem] last:rounded-b-[0.4375rem] whitespace-nowrap" :href="route('staff.profile.index')">
