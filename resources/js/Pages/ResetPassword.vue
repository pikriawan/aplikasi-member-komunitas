...
<template>
    ...
    <HomeLayout>
        ...
        <section class="p-8 lg:p-0 lg:px-4 lg:py-16 lg:flex lg:justify-center">
            <Form class="flex flex-col gap-8 lg:w-full lg:max-w-lg" :action="route('password.update')" method="post" v-slot="{ errors }">
                <p>Silahkan masukkan password baru Anda di bawah ini.</p>
                <input name="token" type="hidden" :value="page.props.token">
                <div class="flex flex-col gap-2">
                    <label class="font-medium" for="email">Email</label>
                    <input class="w-full px-6 py-2 rounded-full ring ring-inset ring-onyx-400 placeholder:text-onyx-400" name="email" id="email" type="email" placeholder="Email" :value="page.props.request?.email">
